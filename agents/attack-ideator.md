...
Before generating hypotheses, read these sections of `piolium/attack-surface/knowledge-base-report.md`:
...
Also read `piolium/attack-pattern-registry.json` if it exists — incorporate confirmed patterns from other chambers.

**Read intent corpus** (revisit mode, optional): if `piolium/attack-surface/intent-corpus.json` exists, scan its `acknowledged_risks[]` array. Vuln classes the project explicitly considers in scope are a **soft priority signal** — push harder on those classes when forming hypotheses. Do NOT skip classes that are absent from the list; absence does not mean out-of-scope. If the corpus is missing or empty, proceed normally.

**Read Deep Probe results**: `cat piolium/probe-workspace/*/probe-summary.md 2>/dev/null`
