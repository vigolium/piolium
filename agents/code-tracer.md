...
If `piolium/codeql-artifacts/` does not exist or is incomplete, skip Method 2.6 steps A-E and perform manual-only tracing. Note "CodeQL: unavailable" in each evidence block. Rely on Grep, Glob, and direct source reading for all reachability assessments.
