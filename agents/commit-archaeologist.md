...
- Standard paths: `auth/`, `piolium/`, `crypto/`, `validation/`, `sanitize/`, `middleware/`, `permission/`, `access/`, `login/`, `session/`, `token/`
...
For each result: `git show --stat <SHA>` and check if paths include auth/payment/crypto (baseline) or Step 1 discovered paths.
...
3. Dedup with advisory-hunter: if `piolium/attack-surface/knowledge-base-report.md` `## Advisory Intelligence` section exists, extract SHAs already recorded there and remove them
...
### File 1: `piolium/attack-surface/commit-recon-report.md`
...
**Scan depth**: up to `$MAX_COMMITS` commits within `$MAX_AGE` across all refs (env: `PIOLIUM_COMMIT_SCAN_LIMIT=<N>`, `PIOLIUM_COMMIT_SCAN_SINCE=<duration>`)
...
### File 2: Append `## Commit Archaeology` to `piolium/attack-surface/knowledge-base-report.md`

Include:
- Priority Commits table (top 30)
- Project security vocabulary discovered (for Phase 3 KB builder context)
- Cross-reference: `piolium/attack-surface/commit-recon-report.md`
- Phase 2 candidate SHAs (for `@patch-bypass-checker` with `type: undisclosed-fix`)
- Phase 5 candidate files/components (HIGH-risk commit paths → attack surface hints)

If `piolium/attack-surface/knowledge-base-report.md` does not exist, create it. If it exists, append the section.
