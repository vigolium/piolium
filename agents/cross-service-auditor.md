...
For each call site, extract the URL string (literal or template). Match against endpoint paths discovered by authz-auditor (`piolium/attack-surface/authz-matrix.md`) or probe-workspace entry-point catalogues. Build edges: `serviceA:file:line → serviceB:handler`.
...
Write `piolium/attack-surface/cross-service-edges.json`:
...
Also write a human-readable summary to `piolium/attack-surface/cross-service-edges.md` listing each edge in a table.
...
Write drafts to `piolium/findings-draft/p8-<NNN>-<slug>.md`.
...
Append to `piolium/attack-surface/knowledge-base-report.md`:
...
- Coverage gaps: <unresolved templates / unmatched channels> — see `piolium/attack-surface/cross-service-edges.md`
