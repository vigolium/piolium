...
- **Output path**: where to write the corpus JSON (`piolium/confirm-workspace/intent-corpus.json` or `piolium/attack-surface/intent-corpus.json` depending on mode).
- **Findings inventory path** (optional): `piolium/confirm-workspace/findings-inventory.json`. If present, you also run a **cross-check pass** (see Step 4) and write per-finding verdicts.
...
Skip generated, vendored, and lockfile directories: `node_modules/`, `vendor/`, `.git/`, `dist/`, `build/`, `target/`, `piolium/` itself.
