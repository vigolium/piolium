...
Follow `~/.config/piolium/skills/audit/references/real-env-validation.md` for provisioning:
...
Evidence capture:
```bash
# Required files in piolium/findings/<ID>-<slug>/evidence/
...
Write back to the finding draft at `piolium/findings/<ID>-<slug>/draft.md`:
```
...
These fields drive the confirm-mode pipeline AND give Phase 14's finding-reporter the PoC status it needs to write an accurate `Proof of Concept (PoC)` section:
...
Do NOT write `piolium/findings/<ID>-<slug>/report.md`. Phase 14's finding-reporter owns that file — your job stops once the PoC, evidence, and draft metadata are in place.
