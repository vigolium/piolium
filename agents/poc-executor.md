...
effort: low
description: Confirmation phase V4 PoC execution agent that runs existing PoC scripts from piolium/findings/ against the live application environment or a remote target, adapts connection details, captures execution evidence, and updates finding confirmation status
---
...
Write the adapted script to `piolium/findings/<ID>-<slug>/confirm-evidence/poc-adapted.{ext}`.
...
```bash
mkdir -p piolium/findings/<ID>-<slug>/confirm-evidence/

cat > piolium/findings/<ID>-<slug>/confirm-evidence/env-info.txt <<EOF
Target: $BASE_URL
Timestamp: $(date -u +%Y-%m-%dT%H:%M:%SZ)
Method: $(jq -r '.method_used' piolium/confirm-workspace/env-connection.json)
...
restore_snapshot() {
  ...
  spec=piolium/confirm-workspace/snapshot-spec.json
  [ -f "$spec" ] || return 0
...
run_variant() {
  ...
  echo "--- variant ${variant_idx} @ $(date -u +%Y-%m-%dT%H:%M:%SZ) ---" \
    >> piolium/findings/<ID>-<slug>/confirm-evidence/attempts.log
  timeout --kill-after=5s 30s <interpreter> "$script" \
    2>&1 | tee -a piolium/findings/<ID>-<slug>/confirm-evidence/attempts.log
}

restore_snapshot
run_variant 1 piolium/findings/<ID>-<slug>/confirm-evidence/poc-adapted.{ext} \
  > piolium/findings/<ID>-<slug>/confirm-evidence/exploit.log
```
...
PoCs built by `poc-builder` (Phase 13) MUST emit a final JSON line on stdout:
...
For **failed** results after both variants: run the `fp-check` skill on the original draft (`piolium/findings/<ID>-<slug>/draft.md`) using the live evidence as context. Two outcomes:
...
Record each attempt and the fp-check verdict in `piolium/findings/<ID>-<slug>/confirm-evidence/attempts.log`.
...
Confirm-Evidence: piolium/findings/<ID>-<slug>/confirm-evidence/
