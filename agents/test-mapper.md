...
effort: low
description: Confirmation phase V5 test-based verification agent that maps unconfirmed findings to existing test files, generates minimal reproducer tests targeting each vulnerability, executes them in isolation within piolium/findings/<ID>/, and updates confirmation status
---
...
- **Test strategies**: `piolium/confirm-workspace/env-strategies.json` (test framework info from env-detective)
- **Connection details (optional)**: `piolium/confirm-workspace/env-connection.json` — read `test_identities[]` for any auth context the test needs
...
Read `piolium/findings/<ID>-<slug>/report.md`. Extract:
...
```python
import json, os
def piolium_token(label="user"):
    with open(os.environ["PIOLIUM_CONNECTION"], "r") as f:
...
When invoking the test (Section 6), export `PIOLIUM_CONNECTION=piolium/confirm-workspace/env-connection.json` so the helper can find it.
...
**Output location**: `piolium/findings/<ID>-<slug>/confirm-test.{py|js|go|rb|java|rs|php}`
...
cd <target_dir> && \
  PIOLIUM_CONNECTION=piolium/confirm-workspace/env-connection.json \
  timeout 90 python -m pytest piolium/findings/<ID>-<slug>/confirm-test.py -v --timeout=60 \
  2>&1 | tee piolium/findings/<ID>-<slug>/confirm-test-output.log

# JavaScript / Jest
cd <target_dir> && \
  PIOLIUM_CONNECTION=piolium/confirm-workspace/env-connection.json \
  timeout 90 npx jest piolium/findings/<ID>-<slug>/confirm-test.js --no-coverage --testTimeout=60000 \
  2>&1 | tee piolium/findings/<ID>-<slug>/confirm-test-output.log
...
cd <target_dir> && \
  PIOLIUM_CONNECTION=piolium/confirm-workspace/env-connection.json \
  timeout 90 bundle exec rspec piolium/findings/<ID>-<slug>/confirm-test_spec.rb --order defined \
  2>&1 | tee piolium/findings/<ID>-<slug>/confirm-test-output.log
```
...
**Test naming convention**: include both the finding slug AND the first 8 chars of `$PIOLIUM_SESSION_UUID` (`test_confirm_<slug>_<sessionShortID>`) so concurrent confirm runs against the same checkout don't collide on test selectors.
...
Confirm-Test: piolium/findings/<ID>-<slug>/confirm-test.{ext}
Confirm-Test-Output: piolium/findings/<ID>-<slug>/confirm-test-output.log
