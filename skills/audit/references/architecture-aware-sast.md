...
Document the split in the `## Static Analysis Summary` section of `piolium/attack-surface/knowledge-base-report.md`.
...
8. Store artifacts under `piolium/codeql-queries/`. Store slice reachability queries as
   `piolium/codeql-queries/slice-<name>.ql` — distinct from security-finding queries; their
...
`piolium/codeql-artifacts/db/`. The purpose is structural intelligence — not security findings.
...
All outputs go to `piolium/codeql-artifacts/`:
...
codeql query run \
  ...
  --output=piolium/codeql-artifacts/entry-points.bqrs \
  -- piolium/codeql-queries/list-sources.ql

codeql bqrs decode \
  --format=json \
  --output=piolium/codeql-artifacts/entry-points.json \
  piolium/codeql-artifacts/entry-points.bqrs
```
...
Run the sink enumeration query for the detected language. Decode to `piolium/codeql-artifacts/sinks.json`.
Group results by `kind` field.

### Step 3: Call graph slice queries

For each high-risk DFD slice in `piolium/attack-surface/knowledge-base-report.md` under
...
QL templates as a starting point. Store queries at `piolium/codeql-queries/slice-<name>.ql`.
...
```bash
codeql database analyze piolium/codeql-artifacts/db/ \
  --format=sarif-latest \
  --output=piolium/codeql-artifacts/flow-paths-raw.sarif \
  --threads=0 \
  --threat-model all
```
...
`piolium/codeql-artifacts/flow-paths-all-severities.md` with sections per rule category. This
...
write them into the `## CodeQL Structural Analysis` section of `piolium/attack-surface/knowledge-base-report.md`.
...
`piolium/attack-surface/knowledge-base-report.md` from the JSON artifacts:
...
`piolium/attack-surface/knowledge-base-report.md`. The Phase 4 enrichment substep, Phase 10, and Phase 12 fall back to pure manual analysis.
...
7. Store artifacts under `piolium/semgrep-rules/`.
...
6. Record any batching, throttling, or narrowed scope in the `## Static Analysis Summary` section of `piolium/attack-surface/knowledge-base-report.md`.
