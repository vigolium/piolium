...
mkdir -p piolium/ piolium/findings-draft/
...
if [ -f piolium/audit-state.json ]; then
  # Append new entry to existing audits array
  python3 -c "
import json, sys
data = json.load(open('piolium/audit-state.json'))
data['audits'].append(json.loads(sys.argv[1]))
json.dump(data, open('piolium/audit-state.json', 'w'), indent=2)
" "$NEW_ENTRY"
else
  # Create new file with first entry
  python3 -c "
import json, sys
json.dump({'audits': [json.loads(sys.argv[1])]}, open('piolium/audit-state.json', 'w'), indent=2)
" "$NEW_ENTRY"
fi
```
...
**Output**: `## Advisory Intelligence` section of `piolium/attack-surface/knowledge-base-report.md`, populated with advisory inventory, architecture intelligence, vulnerability class patterns, and supply chain risk summary.

---

## Phase 2: Patch Bypass Analysis

**Subagent**: `patch-bypass-checker` (one instance per patch)

**Goal**: Determine whether each security patch is sound, bypassable, or relocated.

**Invocation**: spawn one `patch-bypass-checker` instance per patch commit. Each instance receives:
- The patch diff (`git show <commit>`)
- The advisory metadata (CVE/GHSA ID, severity, description)
- The repository path

**Parallelism**: multiple patch-bypass-checker instances can run in parallel since they are read-only.

**Output**: `## Bypass Analysis` section of `piolium/attack-surface/knowledge-base-report.md`

---

## Phase 3: Knowledge Base

**Subagent**: `knowledge-base-builder`
...
Write results to the `## Domain Attack Research` section of `piolium/attack-surface/knowledge-base-report.md`.

**Output**: `piolium/attack-surface/knowledge-base-report.md` with all Phase 3 sections populated (Project Classification, Architecture, Trust Boundaries, DFD/CFD Slices, Threat Model, Attack Surface, Domain Attack Research, Specs/RFCs, Dependencies, Phase 4 Modeling Targets)
...
- store CodeQL artifacts under `piolium/codeql-queries/`
- store Semgrep artifacts under `piolium/semgrep-rules/`
...
rm -rf piolium/codeql-db/ piolium/codeql-db-*/
...
jq -s '{ version: "2.1.0", runs: [ .[].runs[] ] }' \
  piolium/codeql-res/*.sarif \
  piolium/semgrep-res/*.sarif \
  > piolium/merged-results.sarif
```

**Output**: `## Static Analysis Summary` and `## GitHub Actions Audit` sections of
`piolium/attack-surface/knowledge-base-report.md`. The Static Analysis Summary must record:
...
Write enrichment verdicts to the `## SAST Enrichment` section of `piolium/attack-surface/knowledge-base-report.md`.
...
`piolium/attack-surface/knowledge-base-report.md`. Use the Mode C attack taxonomy and manual review checklist
...
**Output**: `## Spec Gap Analysis` section of `piolium/attack-surface/knowledge-base-report.md`. If no specs were identified in Phase 3, mark "None identified" and skip.

---

## Phase 10: Review Chamber Deep Bug Hunting

**Agents**: `chamber-synthesizer`, `attack-ideator`, `code-tracer`, `devils-advocate`, `variant-scout` (optional)
...
**Input**: `piolium/attack-surface/knowledge-base-report.md` (all sections from phases 1-6),
`piolium/codeql-artifacts/` (structural artifacts from Phase 4)
...
5. Create `piolium/chamber-workspace/` and `piolium/attack-pattern-registry.json`
...
```bash
mkdir -p piolium/chamber-workspace/<chamber-id>/{evidence,variant-candidates}
```
...
`piolium/attack-pattern-registry.json` stores confirmed patterns with detection signatures
...
**Output**: `piolium/findings-draft/p7-<NNN>-<slug>.md` (Medium+ only),
`piolium/chamber-workspace/<chamber-id>/debate.md` (audit artifacts),
...
`piolium/findings-draft/p7-*.md` file immediately. Add:
...
**Full review output**: `piolium/adversarial-reviews/<slug>-review.md` using the template
from `report-templates.md`.

**Output**: updated `piolium/findings-draft/` files (CRITICAL/HIGH with cold verification
verdicts), `piolium/adversarial-reviews/<slug>-review.md` per CRITICAL/HIGH VALID finding
...
1. Read the matching pattern from `piolium/attack-pattern-registry.json`
...
4. Read `piolium/chamber-workspace/*/variant-candidates/` for pre-identified candidates from
   Variant Scouts
...
`piolium/findings-draft/p9-<NNN>-<slug>.md` using the finding draft template. Update the
attack pattern registry with each confirmed variant.

**Output**: `piolium/findings-draft/p9-<NNN>-<slug>.md` (one file per variant, Medium or higher only)
...
1. List all files in `piolium/findings-draft/` with `Verdict: VALID`.
...
3. For each confirmed draft, create `piolium/findings/<ID>-<slug>/` and copy the draft as the basis for the `vuln-report` output.
...
**Real-environment execution mandate for CRITICAL/HIGH findings**: For every CRITICAL or HIGH finding promoted to `piolium/findings/`, PoC execution in a real environment is required before the final report is generated. Reuse the Stage 2 adversarial environment if it was successfully provisioned; otherwise provision a new environment following `real-env-validation.md`.

Capture evidence in `piolium/findings/<ID>-<slug>/evidence/`:
```
piolium/findings/<ID>-<slug>/evidence/
...
-   **Naming Convention**: Save each report to `piolium/findings/<ID>-<slug>/report.md`.
...
This is the mandatory final step to synthesize the entire audit. Generate `piolium/final-audit-report.md` using the template in `audit/references/report-templates.md`.
...
```bash
ls piolium/attack-surface/knowledge-base-report.md piolium/final-audit-report.md piolium/findings/
```
