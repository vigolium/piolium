...
- Only role that writes finding drafts to `piolium/findings-draft/`
...
Within piolium-claude: agents communicate via the shared `debate.md` file AND `SendMessage`.
...
Within piolium-codex: agents poll `debate.md` for new sections (file-based coordination).
...
`piolium/chamber-workspace/<chamber-id>/variant-candidates/` for the Synthesizer to decide
...
**Finding draft written to**: piolium/findings-draft/p7-<NNN>-<slug>.md
