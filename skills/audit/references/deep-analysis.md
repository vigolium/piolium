...
Open `piolium/codeql-artifacts/call-graph-slices.json` and find the entry for the slice.
...
Open `piolium/codeql-artifacts/flow-paths-all-severities.md`. Filter to rules and file paths
...
The `## CodeQL Structural Analysis` section of `piolium/attack-surface/knowledge-base-report.md` contains
...
The database at `piolium/codeql-artifacts/db/` is live until Phase 12 completes. When a manual
...
Store reusable on-demand queries at `piolium/codeql-queries/on-demand-<slug>.ql`. These become
...
the `## Static Analysis Summary` section of `piolium/attack-surface/knowledge-base-report.md` documents `--build-mode=none` or low extraction quality,
...
- First, read the `## Domain Attack Research` section of `piolium/attack-surface/knowledge-base-report.md`.
