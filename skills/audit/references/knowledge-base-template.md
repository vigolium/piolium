...
Unified template for `piolium/attack-surface/knowledge-base-report.md`. This is the single knowledge repository
...
For re-audit: load the existing `piolium/attack-surface/knowledge-base-report.md` as the starting point. Update
...
*Phase 3 — knowledge-base-builder*
...
*Phase 3 — knowledge-base-builder (used by Phase 9)*
...
*Phase 2 — patch-bypass-checker (one instance per patch)*
...
*Phase 4 — static-analyzer (structural extraction sub-step)*
...
*Derived from `piolium/codeql-artifacts/flow-paths-all-severities.md`.*
...
|----------|------|-------------|--------------|
| `piolium/codeql-queries/<name>.ql` | CodeQL | <DFD/CFD slice> | N |
| `piolium/semgrep-rules/<name>.yaml` | Semgrep | <DFD/CFD slice> | N |
...
*Phase 10 — deep-auditor (forward-append only; Phase 3 content preserved for auditability)*
...
## Notes for knowledge-base-builder
...
- The `## CodeQL Structural Analysis` section (including diagrams) is populated by the Phase 4
  static analyzer, not by the knowledge-base-builder. The knowledge-base-builder's contribution
