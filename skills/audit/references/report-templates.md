...
`piolium/audit-state.json` is an append-only audit history. Each audit run is appended as a new
...
load the KB sections from `piolium/attack-surface/knowledge-base-report.md` as the starting knowledge base.

For Phase 4, `reports_generated` must include `piolium/codeql-artifacts/entry-points.json`,
`piolium/codeql-artifacts/sinks.json`, `piolium/codeql-artifacts/call-graph-slices.json`, and
`piolium/codeql-artifacts/flow-paths-all-severities.md`, plus the `## Static Analysis Summary`
section written to `piolium/attack-surface/knowledge-base-report.md`. Missing any causes `validation_passed: false`.
...
"codeql_structural": {
  ...
  "db_path": "piolium/codeql-artifacts/db/"
}
```

## Finding Draft Template

Used for `piolium/findings-draft/<phase>-<NNN>-<slug>.md` files written incrementally during Phases 7-9.
...
Debate: <path to chamber debate transcript, e.g., piolium/chamber-workspace/chamber-01/debate.md>
...
- **Capture evidence.** For Critical and High findings, save execution output to `piolium/findings/<ID>-<slug>/evidence/` (screenshots, response captures, or log snippets).
...
Used for `piolium/adversarial-reviews/<slug>-review.md` files written during Phase 11 Stage 2.

```markdown
# Adversarial Review: [Finding Title]

Finding-Ref: piolium/findings-draft/<phase>-<NNN>-<slug>.md
...
Setup commands: see `piolium/real-env-evidence/<slug>/setup.sh`
...
Evidence: piolium/real-env-evidence/<slug>/
...
## Pentest-Style Final Report Template (`piolium/final-audit-report.md`)
...
- **Detailed Report:** [piolium/findings/[ID]-[slug]/report.md]
- **Proof of Concept:** [piolium/findings/[ID]-[slug]/poc.py]
...
```bash
jq 'length' piolium/codeql-artifacts/entry-points.json
jq 'length' piolium/codeql-artifacts/sinks.json
jq '[.[] | select(.reachable == true)] | length' piolium/codeql-artifacts/call-graph-slices.json
jq '.runs[0].results | length' piolium/codeql-artifacts/flow-paths-raw.sarif
```
...
File: `piolium/attack-pattern-registry.json`
...
File: `piolium/chamber-workspace/<chamber-id>/debate.md`
