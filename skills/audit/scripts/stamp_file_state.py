...
piolium/file-state.json — a per-file lattice that records which audit IDs
have touched which file. Run this as a side-effect of the final phase of
any audit (balanced Phase 8, deep Phase 15) so the next audit can compute
...
Usage:
    stamp_file_state.py [--target <path>] [--piolium-dir <path>] [--audit-id <id>] [--phases <n,n,...>]

Defaults:
    --target       cwd
    --piolium-dir   <target>/piolium
    --audit-id     read from <piolium-dir>/audit-state.json's last entry
    --phases       all integer keys from the last audit's `phases` map

Excludes:
    Standard noise (.git/, node_modules/, vendor/, __pycache__/, dist/,
    build/, .venv/, target/, .piolium-merge-staging-*/) and the piolium/
...
EXCLUDED_DIR_PREFIXES = (".piolium-merge-staging-", "bak-piolium-")
...
def detect_audit_id(piolium_dir: Path) -> Optional[tuple[str, list[int]]]:
    ...
    """
    audit_state = piolium_dir / "audit-state.json"
...
def stamp(
    ...
    state_path = piolium_dir / "file-state.json"
...
def main() -> None:
    ...
    parser.add_argument("--piolium-dir", default=None, help="Piolium data dir (default: <target>/piolium)")
    ...
    piolium_dir = Path(args.piolium_dir) if args.piolium_dir else target / "piolium"
    piolium_dir.mkdir(parents=True, exist_ok=True)

    audit_id = args.audit_id
    phases: list[int] = parse_phases_arg(args.phases or "")

    if not audit_id or not phases:
        detected = detect_audit_id(piolium_dir)
    ...
    try:
        counts = stamp(target, piolium_dir, audit_id, phases, max_bytes=args.max_bytes)
    except OSError as exc:
